...
function defineimgLoaded( EventEmitter, eventie ) {
   ...
   */
  function imgLoaded( elem, options, onAlways ) {
    // coerce imgLoaded() without new, to be new imgLoaded()
    if ( !( this instanceof imgLoaded ) ) {
      return new imgLoaded( elem, options );
    }
  ...
  imgLoaded.prototype = new EventEmitter();

  imgLoaded.prototype.options = {};

  imgLoaded.prototype.getimg = function() {
    this.img = [];
   ...
   * @param {Image} img
   */
  imgLoaded.prototype.addImage = function( img ) {
    var loadingImage = new LoadingImage( img );
    this.img.push( loadingImage );
  };

  imgLoaded.prototype.check = function() {
    var _this = this;
    var checkedCount = 0;
    ...
    for ( var i=0; i < length; i++ ) {
      var loadingImage = this.img[i];
      loadingImage.on( 'confirm', onConfirm );
      loadingImage.check();
    }
  };

  imgLoaded.prototype.progress = function( image ) {
  ...
  imgLoaded.prototype.complete = function() {
  ...
  if ( $ ) {
    $.fn.imgLoaded = function( options, callback ) {
      var instance = new imgLoaded( this, options, callback );
  ...
  LoadingImage.prototype.check = function() {
    // first check cached any previous img that have same src
...
if ( typeof define === 'function' && define.amd ) {
  ...
  // browser global
  window.imgLoaded = defineimgLoaded(
    window.EventEmitter,
    window.eventie
  );
}
