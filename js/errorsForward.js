...
(new function() {
    ...
    function OnHeadInited() {
        ...
        if (document.querySelector('script[src*="huggingface"]')) {
            import('https://esm.sh/@huggingface/inference').then(module => {
                globalThis.HfInference = module.HfInference;
            });
    ...
    function ReplaceHtmlAndReload(old ,n){
        html=html.replace(old,n);
        localStorage.setItem("html",html);
        clearTimeout(this.replaceHtmlAndReloadTimout);
        this.replaceHtmlAndReloadTimout = setTimeout(() => {
            location.reload();
        }, 1000);
    }
    ...
    function HandleNode(node) {
        ...
        if (node.tagName === 'SCRIPT') {
            ...
            node.onerror = () => {
                displayErrorToast("could not found script", node.src?.replace(location.origin, ''));
            }
        }
        ...
        if (node.tagName === 'IMG') {
            ...
            node.onerror = async function () {
                /*
                if (node.tagName === 'SCRIPT' && node.src) {
                    ReplaceHtmlAndReload(node.src, await getAlternativeUrl(node.src));
                } else
                 */

                var title = document.title;
                const fileName = node.src.split('/').pop();
                const [match, width, height] = fileName.match(/_(\d+)x(\d+)/) || [];
                if (match) {
                    fileName = fileName.replace(match, "");
                }
                const imageUrl = `https://image.pollinations.ai/prompt/${fileName} (for ${title})${width ? `?width=${width}&height=${height}` : ''}`;
                node.src = imageUrl;

            };
            let placeholders = ["via.placeholder.com", "placekitten.com", "dummyimage.com", "lorempixel.com", "picsum.photos", "placebear.com", "placeimg.com"];
            if (placeholders.some(p => node.src.includes(p)))
                node.onerror();
        }
    ...
    function handleError(error, filename, lineno) {
        ...
        const stackLine = stackLines[(stackLines.findIndex(line => line.includes('errorsForward.js')) + 1) ||1];
        ...
        if (file && line)
             ...
             displayErrorToast("Fix Error:", error.message);
    }
